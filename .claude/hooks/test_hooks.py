...
COMMAND_CASES = [
    ...
    (ASK,  "kubectl rollout undo deployment/api",                   "rollout undo"),
    (ASK,  "kubectl set image deployment/api api=repo:v2",          "set image (ships a version)"),
    (ASK,  "kubectl autoscale deployment/api --min=2 --max=10",     "autoscale"),
    (ASK,  "kubectl expose deployment/api --port=80",               "expose"),
    (ASK,  "kubectl annotate pod foo key=val",                      "annotate"),
    ...
    (PASS, "kubectl rollout status deployment/api",                 "rollout status (read-only)"),
    (PASS, "kubectl rollout history deployment/api",                "rollout history (read-only)"),
